...
<script>
  const aboutStoreUrl = @json(route('admin.aboutus.store'));
  ...
  const aboutState = {
    active: 'umum',
    search: '',
    umum: (@json($generalProfile ?? []) || []).map((item) => normalizeAboutItem(item, 'general')),
    struktur: (@json($strukturData ?? []) || []).map((item) => normalizeAboutItem(item, 'structure')),
    deletingId: null,
    editingId: null,
    editingSection: null,
    isDeleting: false,
