        ...
        <div class="bg-white rounded-3xl shadow-2xl shadow-slate-200/60 border border-slate-100 p-10">
            ...
            @if ($errors->any())
                <div class="mb-6 flex items-start gap-2 rounded-xl border border-rose-100 bg-rose-50 px-4 py-3">
                    ...
                    <p class="text-xs font-bold text-rose-700">{{ $errors->first() }}</p>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.force-change-password.update') }}" class="space-y-5" id="forceChangeForm">
                @csrf
            ...
            <p class="mt-6 text-center text-xs text-slate-400">
                Masuk sebagai <strong class="text-slate-600">{{ Auth::user()->name }}</strong> ({{ Auth::user()->email }})
                &bull; <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-force').submit();" class="text-rose-500 hover:text-rose-600 font-bold">Logout</a>
            </p>
            <form id="logout-form-force" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
        </div>
    </div>
</div>
