          ...
          @if($filterOpen)
            <div class="absolute top-full right-0 mt-2 w-48 bg-white border border-slate-100 rounded-xl shadow-2xl overflow-hidden z-20 py-1">
              @foreach($filterOptions as $option)
                <button onclick="selectFilter('{{ $option }}')" class="w-full text-left px-5 py-3 text-xs font-bold transition-all {{ $selectedFilter === $option ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                  {{ $option }}
                </button>
              @endforeach
            </div>
...
<script>
  let filterOpen = {{ $filterOpen ? 'true' : 'false' }};
  ...
  async function loadDonationChart() {
    try {
      const response = await fetch('{{ route("admin.api.donation-trend") }}');
