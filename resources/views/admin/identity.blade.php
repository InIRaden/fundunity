      ...
      <div class="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 flex flex-col items-center text-center">
        <div class="relative group cursor-pointer mb-6">
          <div class="w-32 h-32 rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center p-4 overflow-hidden">
            @if (!empty($identity['logoUrl']))
              <img id="identityLogoPreview" src="{{ $identity['logoUrl'] }}" alt="Logo organisasi" class="w-full h-full object-contain rounded-xl">
        ...
        <h2 id="identityPreviewOrgName" class="text-lg font-bold text-slate-900 leading-tight">{{ $identity['orgName'] }}</h2>
...
<script>
  ...
  const identityUpdateUrl = @json(route('admin.identity.update'));
