<div class="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-30 w-full">
  ...
  <div class="flex items-center gap-4">
    <div>
      <h1 class="text-base font-bold text-slate-900 leading-tight tracking-tight">{{ $pageMeta['title'] ?? 'Admin Panel' }}</h1>
      <p class="text-xs text-slate-400 mt-0.5">{{ $pageMeta['subtitle'] ?? '' }}</p>
    </div>
  </div>
  ...
  <div class="flex items-center gap-3">
    <!-- Avatar → Settings -->
    <div class="flex items-center gap-2 cursor-pointer group" onclick="window.location.href='{{ route('admin.settings') }}'">
      <p class="text-xs font-bold text-slate-700 hidden sm:block group-hover:text-emerald-700 transition-colors">{{ auth()->user()?->name ?? 'Admin' }}</p>
      <div class="relative">
        ...
        <div class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-white rounded-full border border-slate-200 flex items-center justify-center">
          <i class="ph ph-gear-six text-[9px] text-slate-400 group-hover:text-emerald-600 leading-none"></i>
        </div>
      </div>
    </div>
  </div>
