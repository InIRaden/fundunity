...
<div class="relative">
  ...
  <div id="adminSidebar" class="fixed top-0 left-0 bottom-0 flex flex-col transition-all duration-300 ease-in-out {{ $sidebarBgClass }} text-white rounded-r-3xl w-56 z-40">
    ...
    <nav class="flex-1 space-y-0.5 px-0 pt-2 pb-2 overflow-y-auto overflow-x-hidden relative hide-scrollbar">
      @foreach($menuItems as $item)
        ...
        @if($enabled)
          <a href="{{ route($item['route']) }}" class="sidebar-item relative group flex items-center gap-3 py-2 transition-all duration-150 cursor-pointer {{ $isActive ? 'bg-gray-50 text-emerald-600 shadow-sm active-sidebar-item' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
            @php
            ...
            <span class="text-[13px] font-semibold tracking-wide whitespace-nowrap sidebar-text-show {{ $isActive ? 'text-emerald-600 font-bold' : '' }}">{{ $item['label'] }}</span>
          </a>
        @endif
      @endforeach
      ...
      @if(Auth::user()?->isSuperAdmin() && (string)($siteSettings['admin_menu_management_enabled'] ?? '1') === '1')
        ...
        @endphp
        <a href="{{ route('admin.management') }}" class="sidebar-item relative group flex items-center gap-3 py-2 transition-all duration-150 cursor-pointer {{ $isManagementActive ? 'bg-gray-50 text-emerald-600 shadow-sm active-sidebar-item' : 'text-amber-200 hover:bg-white/10 hover:text-white' }}">
          @php
            $iconClass = $isManagementActive ? 'ph-fill ph-crown' : 'ph ph-crown';
          @endphp
          <i class="{{ $iconClass }} shrink-0 text-[19px] leading-none transition-all"></i>
          <span class="text-[13px] font-semibold tracking-wide whitespace-nowrap sidebar-text-show {{ $isManagementActive ? 'text-emerald-600 font-bold' : '' }}">Manajemen Admin</span>
        </a>
      @endif
    </nav>
