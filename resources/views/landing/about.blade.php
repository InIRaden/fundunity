            ...
            <div class="flex flex-col md:flex-row items-center gap-16">
                <div class="w-full md:w-1/2 relative">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <img src="{{ $primaryImage }}" alt="Relawan" class="rounded-3xl shadow-lg w-full h-64 object-cover object-center translate-y-8">
                        <img src="{{ $secondaryImage }}" alt="Anak-anak" class="rounded-3xl shadow-lg w-full h-64 object-cover object-center">
                    </div>
                ...
                <div class="w-full md:w-1/2">
                    ...
                    <p class="text-slate-600 text-lg leading-relaxed mb-8">
                        {{ optional($page)->story_content ?? 'Kami adalah organisasi kemahasiswaan dan sosial kultural yang berfokus membangun gerakan solutif bernilai tinggi, transparan, serta berdampak nyata bagi masyarakat luas.' }}
                    </p>
                    ...
                    <div data-about-panel="vision" class="about-tab-panel min-h-[150px] space-y-4 animate-fade-in">
                        <div class="flex gap-4">
                            ...
                            <div>
                                <h4 class="font-bold text-slate-800 mb-1">{{ $visionItem?->title ?? (optional($page)->vision_title ?? 'Visi Kami') }}</h4>
                                <p class="text-sm text-slate-500 leading-relaxed">
                                    {{ $visionItem?->description ?? (optional($page)->vision_content ?? 'Menjadi jembatan kebaikan digital nomor satu yang transparan dan dapat diandalkan oleh masyarakat luas.') }}
                                </p>
                            </div>
                        </div>
                        <div class="flex gap-4">
                            <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 256 256" class="text-emerald-500 shrink-0 mt-0.5" height="24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm45.66,85.66-56,56a8,8,0,0,1-11.32,0l-24-24a8,8,0,0,1,11.32-11.32L112,148.69l50.34-50.35a8,8,0,0,1,11.32,11.32Z"></path></svg>
                            <div>
                                <h4 class="font-bold text-slate-800 mb-1">{{ $missionItem?->title ?? (optional($page)->mission_title ?? 'Misi Utama') }}</h4>
                                <p class="text-sm text-slate-500 leading-relaxed">
                            ...
                            @forelse($teamPreview as $member)
                                <div class="bg-white border border-slate-100 rounded-xl p-4 flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-full overflow-hidden shrink-0 border-2 border-emerald-100 bg-slate-50">
                                        <img src="{{ $member->image_url ?: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&h=150&fit=crop' }}" alt="{{ $member->title }}" class="w-full h-full object-cover">
                                    </div>
                                    <div>
                                        <p class="about-clamp-1 text-sm font-bold text-slate-900">{{ $member->title }}</p>
                                        <p class="about-clamp-1 text-[10px] uppercase font-bold text-emerald-600">{{ $member->position ?? 'Tim Organisasi' }}</p>
                                    </div>
                                </div>
                            @empty
