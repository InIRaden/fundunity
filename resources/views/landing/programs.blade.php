        ...
        <div class="flex flex-col items-center gap-4 rounded-[32px] bg-white p-6 shadow-2xl shadow-slate-200/50 md:flex-row">
            ...
            <div class="flex w-full gap-2 overflow-x-auto pb-2 md:w-auto md:pb-0">
                ...
                @foreach($categories as $category)
                    <button type="button" data-filter="{{ $category }}" class="program-filter whitespace-nowrap rounded-2xl bg-slate-50 px-6 py-4 text-sm font-bold text-slate-500 transition-all hover:bg-slate-100">
                        {{ $category }}
                    </button>
                @endforeach
            </div>
            ...
            @forelse($campaigns as $campaign)
                ...
                <article data-card data-title="{{ strtolower($campaign->title) }}" data-category="{{ $campaign->category ?? 'Umum' }}" class="program-card group flex h-full flex-col overflow-hidden rounded-[40px] border border-slate-100 bg-white shadow-xl shadow-slate-200/50 transition-transform duration-300 hover:-translate-y-2">
                    ...
                    <div class="flex flex-1 flex-col p-8">
                        <h3 class="program-clamp-2 mb-6 text-xl font-bold leading-snug text-slate-900 transition-colors group-hover:text-emerald-600">{{ $campaign->title }}</h3>

                        <div class="mt-auto">
                            <div class="mb-3 flex items-end justify-between">
                                <div>
                                    <p class="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-500">Terkumpul</p>
                                    <p class="text-xl font-extrabold leading-none text-emerald-600">Rp {{ number_format((int) $campaign->collected, 0, ',', '.') }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="mb-1 text-[10px] font-black uppercase tracking-widest text-slate-400">Target</p>
                                    <p class="text-sm font-bold leading-none text-slate-600">Rp {{ number_format((int) $campaign->target, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        ...
                        <a href="{{ route('donation.form', ['campaign' => $campaign->id]) }}" class="mt-8 block w-full rounded-2xl bg-emerald-500 py-4 text-center text-sm font-black text-white shadow-lg shadow-emerald-500/30 transition-all hover:bg-emerald-600 active:scale-95">
                            Donasi Sekarang
                        </a>
                    </div>
                </article>
