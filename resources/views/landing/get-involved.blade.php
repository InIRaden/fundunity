...
<div class="relative min-h-[70vh] bg-slate-50 pb-16 pt-24">
    ...
    <div class="relative z-20 mx-auto mb-10 max-w-4xl px-6 pt-8 text-center animate-fade-in">
        <h1 class="text-4xl font-extrabold text-emerald-600 mb-4">Daftarkan Dirimu Sebagai Relawan</h1>
        <p class="text-slate-600 text-md max-w-2xl mx-auto">
            Gabungkan keahlian Anda dengan program kami. Pendaftaran akan ditinjau langsung oleh tim kami untuk mencocokkan Anda dengan program yang paling sesuai.
        </p>
    </div>
    ...
    @if(session('volunteer_success'))
        <div class="max-w-2xl mx-auto bg-white rounded-3xl p-12 text-center shadow-2xl relative z-10 border border-slate-100 animate-slide-up">
            ...
            <p class="mb-8 leading-relaxed text-slate-500">
                {{ session('volunteer_success') ?? 'Pendaftaran Anda berhasil kami terima dan akan masuk ke sistem Admin kami. Koordinator relawan kami akan segera menghubungi Anda melalui Email atau WhatsApp untuk proses onboarding lebih lanjut.' }}
            </p>
            <a href="{{ route('landing.get-involved') }}" class="px-8 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors">
    ...
    @else
        <div class="max-w-3xl mx-auto bg-white rounded-3xl overflow-hidden shadow-2xl relative z-10 border border-slate-100 flex flex-col md:flex-row">
            <div class="md:w-5/12 bg-emerald-600 p-10 text-white flex flex-col justify-center relative overflow-hidden">
                ...
                <h3 class="text-2xl font-extrabold mb-4">Jadilah Bagian dari Perubahan</h3>
                <p class="text-sm text-emerald-100 italic mb-6">"Perubahan terbesar dimulai dari langkah kecil yang diambil bersama."</p>
            </div>

            <form method="POST" action="{{ route('get-involved.store') }}" class="md:w-7/12 p-10 flex flex-col gap-5">
                @csrf
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" required value="{{ old('name') }}" placeholder="Cth: Budi Santoso" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all text-slate-800">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" required value="{{ old('email') }}" placeholder="budi@email.com" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all text-slate-800">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1.5">Nomor WhatsApp</label>
                        <div class="relative">
                            ...
                            <input type="text" name="phone" required value="{{ old('phone') }}" placeholder="+62 8..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all text-slate-800">
                        </div>
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-500 mb-1.5">Bidang Kolaborasi</label>
                    <select name="category" class="w-full px-4 py-2.5 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all text-slate-800 appearance-none">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 mb-1.5">Pesan Tambahan (Opsional)</label>
                    <div class="relative">
                        ...
                        <textarea name="message" rows="3" placeholder="Ceritakan motivasi atau keahlian spesifik Anda..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all text-slate-800 resize-none">{{ old('message') }}</textarea>
                    </div>
                </div>
