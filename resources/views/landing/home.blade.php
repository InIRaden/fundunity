...
<section class="relative flex min-h-screen items-center overflow-hidden bg-slate-900 pb-20 pt-24">
    ...
    <div class="relative z-10 mx-auto flex max-w-7xl flex-col items-center px-6 text-center">
        ...
        <div class="flex w-full flex-col items-center justify-center gap-4 sm:flex-row">
            <a href="{{ route('programs') }}" class="flex w-full items-center justify-center gap-3 rounded-2xl bg-emerald-500 px-8 py-4 text-center text-base font-extrabold text-slate-900 shadow-xl shadow-emerald-500/30 transition-all hover:bg-emerald-400 sm:w-auto">
                Pilih Program Bantuan
                <i class="ph ph-heart-straight text-xl text-rose-500"></i>
            </a>
            <a href="{{ route('about') }}" class="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/10 px-8 py-4 text-center text-base font-bold text-white backdrop-blur-md transition-all hover:bg-white/20 sm:w-auto">
    ...
    <div class="mx-auto max-w-7xl px-6">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            @forelse($homeFocusAreas as $index => $focusArea)
                <div class="group rounded-[2rem] border p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl {{ $focusStyles[$index % count($focusStyles)] }}">
...
<section id="program" class="relative overflow-hidden bg-slate-50 py-24">
    ...
    <div class="relative z-10 mx-auto max-w-7xl px-6">
        <div class="mb-16 flex flex-col justify-between gap-6 md:flex-row md:items-end">
            ...
            <a href="{{ route('programs') }}" class="flex items-center gap-2 whitespace-nowrap font-bold text-emerald-600 transition-colors hover:text-emerald-700">
                Lihat Semua Program
            ...
            @forelse($homeCampaigns as $campaign)
                ...
                <article class="group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-xl shadow-slate-200/50 transition-transform duration-300 hover:-translate-y-2">
                    ...
                    <div class="flex flex-1 flex-col p-6">
                        ...
                        <a href="{{ route('donation.form', ['campaign' => $campaign->id]) }}" class="mt-6 block w-full rounded-xl bg-emerald-500 py-3.5 text-center text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition-colors hover:bg-emerald-600">
                            Donasi Sekarang
                        </a>
                    </div>
                </article>
    ...
    <div class="container mx-auto max-w-4xl overflow-hidden rounded-3xl bg-gradient-to-br from-emerald-600 to-emerald-800 p-8 text-center text-white shadow-xl md:p-12">
        <div class="relative z-10 mx-auto flex max-w-xl flex-col items-center space-y-4">
            ...
            <div class="flex w-full flex-col justify-center gap-4 pt-4 sm:flex-row">
                <a href="{{ route('get-involved') }}" class="rounded-xl bg-white px-6 py-3 text-center text-sm font-bold text-emerald-800 shadow-md transition hover:bg-slate-50">Gabung Jadi Relawan</a>
                <a href="{{ route('faq') }}" class="rounded-xl border border-white/40 px-6 py-3 text-center text-sm font-bold text-white transition hover:bg-white/10">Tanya Jawab (FAQ)</a>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="relative rounded-3xl border border-slate-100 bg-white p-8 shadow-xl shadow-slate-200/50 md:p-10">
            @if(session('success'))
                <div class="animate-fade-in py-16 text-center">
                    ...
                    <a href="{{ route('home') }}#kontak" class="border-b-2 border-emerald-600/30 pb-1 font-bold text-emerald-600 transition-colors hover:text-emerald-700">
                        Kirim Pesan Lainnya
                    </a>
                </div>
            @else
