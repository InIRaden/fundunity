        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($homeFocusAreas as $index => $focusArea)
                ...
                <div class="flex h-full flex-col bg-white p-8 md:p-10 rounded-[40px] shadow-xl shadow-slate-200/50 border border-slate-100 hover:-translate-y-2 transition-transform duration-300">
                    <div class="mb-5 text-[40px] {{ $focusStyle }}">
                        <i class="{{ $areaIcon }}"></i>
                    </div>
                    <h3 class="text-xl font-bold text-slate-900 mb-4">{{ $focusArea->title }}</h3>
                    <p class="text-slate-600 leading-relaxed flex-1">{{ $focusArea->description }}</p>
                </div>
            @empty
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($homeCampaigns as $campaign)
                ...
                <div class="bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-xl shadow-slate-200/50 hover:-translate-y-2 transition-transform duration-300 group flex flex-col h-full">
                    <a href="{{ route('campaign.detail', ['campaign' => $campaign->id]) }}" class="relative h-56 overflow-hidden block">
