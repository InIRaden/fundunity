...
<div class="relative min-h-screen flex items-center pt-24 pb-20 md:pb-32 overflow-hidden bg-slate-900">
    <div class="absolute inset-0">
        <img src="{{ $heroImage }}" alt="Children smiling" class="w-full h-full object-cover opacity-40 object-top">
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10 flex flex-col items-center text-center">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
            <a href="{{ route('landing.programs') }}" class="w-full sm:w-auto px-8 py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-extrabold rounded-2xl transition-all shadow-xl shadow-emerald-500/30 flex items-center justify-center gap-3">
            ...
            <a href="{{ route('landing.about') }}" class="w-full sm:w-auto px-8 py-4 bg-white/10 hover:bg-white/20 text-white font-bold rounded-2xl backdrop-blur-md border border-white/10 transition-all flex items-center justify-center gap-3">
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($homeFocusAreas as $index => $focusArea)
                ...
                @endphp
                <div class="rounded-[2rem] p-8 border {{ $focusStyle }} transition-all duration-300 hover:-translate-y-2 hover:shadow-xl group">
                    <div class="mb-6">{!! $focusIcon !!}</div>
                    <h3 class="font-bold text-slate-900 text-lg mb-2">{{ $focusArea->title }}</h3>
                    <p class="text-slate-500 text-sm leading-relaxed">{{ $focusArea->description }}</p>
                </div>
            @empty
    ...
    <div class="container mx-auto max-w-4xl bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-3xl p-8 md:p-12 text-center text-white relative overflow-hidden shadow-xl">
        ...
        <div class="relative space-y-4 max-w-xl mx-auto flex flex-col items-center">
            ...
            <div class="flex flex-col sm:flex-row gap-4 pt-4 w-full justify-center">
                <a href="{{ route('landing.get-involved') }}" class="px-6 py-3 bg-white text-emerald-800 font-bold rounded-xl shadow-md hover:bg-slate-50 transition text-sm text-center">Gabung Jadi Relawan</a>
                <a href="{{ route('landing.faq') }}" class="px-6 py-3 border border-white/40 hover:bg-white/10 text-white font-bold rounded-xl transition text-sm text-center">Tanya Jawab (FAQ)</a>
            </div>
        </div>
    </div>
</section>
                ...
                <form id="homeContactForm" method="POST" action="{{ route('contact.store') }}" class="flex flex-col gap-6 animate-fade-in">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Nama Anda atau Organisasi" class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required placeholder="alamat@email.com" class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Isi Pesan</label>
                        <textarea name="message" rows="4" required placeholder="Tuliskan tujuan / masalah yang ingin didiskusikan..." class="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all resize-none">{{ old('message') }}</textarea>
                    </div>
