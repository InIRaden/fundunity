            ...
            <div class="mb-12 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                ...
                <div id="galleryFilterControls" class="flex w-full gap-2 overflow-x-auto rounded-2xl bg-slate-100 p-1 md:w-auto">
                    @foreach($categoryTabs as $tab)
                        <button
                            type="button"
                            data-gallery-filter="{{ $tab }}"
                            class="gallery-filter-btn whitespace-nowrap rounded-xl px-6 py-2.5 text-sm font-bold transition-all {{ $loop->first ? 'bg-white text-emerald-600 shadow-md' : 'text-slate-500 hover:text-emerald-600' }}"
                        >
                            {{ $tab }}
                        </button>
                    @endforeach
            ...
            <div id="galleryGrid" class="grid auto-rows-[200px] grid-cols-2 gap-6 lg:grid-cols-4">
                @foreach($galleryEntries as $index => $entry)
                    ...
                    <button
                        type="button"
                        data-gallery-card
                        data-gallery-category="{{ $entry['category'] }}"
                        data-gallery-lightbox="{{ $entry['image'] }}"
