...
<div class="space-y-8 max-w-[1600px] mx-auto w-full mb-10">
    ...
    <?php $__currentLoopData = $stats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/40 transition-all hover:border-admin-200 group">
        <div class="flex justify-between items-start mb-4">
          ...
          <div class="flex items-center text-xs font-bold px-2 py-1 rounded-full <?php echo e($stat['trend'] === 'up' ? 'text-admin-700 bg-admin-50' : 'text-amber-700 bg-amber-50'); ?>">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 p-8 shadow-xl shadow-slate-200/40 flex flex-col h-full">
        ...
        <?php $__currentLoopData = $feedItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="flex gap-4 group">
            <div class="flex flex-col items-center">
              <div class="w-3 h-3 rounded-full shrink-0 border-2 border-white ring-4 ring-slate-50 <?php echo e($item['type'] === 'in' ? 'bg-admin-500' : ($item['type'] === 'out' ? 'bg-amber-500' : 'bg-blue-500')); ?>"></div>
              <?php if($index !== count($feedItems) - 1): ?>
                <div class="w-0.5 h-full bg-slate-100 mt-2"></div>
              <?php endif; ?>
            </div>
            <div class="pb-4">
              <p class="text-sm font-bold text-slate-800 leading-tight mb-1 group-hover:text-admin-600 transition-colors"><?php echo e($item['event']); ?></p>
      ...
      <a href="<?php echo e(route('admin.keuangantransparansi')); ?>" class="w-full mt-4 py-3.5 bg-slate-50 border border-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-admin-50 hover:text-admin-700 hover:border-admin-200 transition-colors shadow-sm text-center">
...
<?php echo $__env->make('layouts.admin.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\kuliah\lain lain\Magang\YMP\iniraden_fundunity\resources\views/admin/home.blade.php ENDPATH**/ ?>