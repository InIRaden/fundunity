...
<div id="campaignModal" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
...
<script>
  const campaignState = {
    campaigns: <?php echo json_encode($campaigns, 15, 512) ?>,
    search: '',
    editingId: null,
    status: 'aktif',
  };
  ...
  function renderStats() {
    ...
    const cards = [
      { label: 'Campaign Aktif', value: `${totalActive} Campaign`, icon: 'ph ph-target', color: 'emerald' },
      { label: 'Total Target (Aktif)', value: rp(totalTarget), icon: 'ph ph-arrow-up-right', color: 'indigo' },
      { label: 'Total Terkumpul (Aktif)', value: rp(totalCollected), icon: 'ph ph-users-three', color: 'amber' },
    ];
    document.getElementById('campaignStats').innerHTML = cards.map(c => `
      <div class="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/40 p-6 flex items-center gap-4 group transition-colors">
        <div class="w-12 h-12 rounded-xl bg-${c.color}-50 border border-${c.color}-100 text-${c.color}-600 flex items-center justify-center shrink-0">
...
<?php echo $__env->make('layouts.admin.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\Magang\PT. YMP\fundunity\resources\views/admin/campaign.blade.php ENDPATH**/ ?>