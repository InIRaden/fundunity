...
<?php $__env->startSection('body-data', 'landing-home'); ?>
...
<div class="relative min-h-screen flex items-center pt-24 pb-20 md:pb-32 overflow-hidden bg-[#022c22]" id="heroSection">
    ...
    <?php if($sliderItems->count() > 1): ?>
        <?php $__currentLoopData = $sliderItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $si => $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="hero-slide absolute inset-0 transition-opacity duration-1000 <?php echo e($si === 0 ? 'opacity-100' : 'opacity-0'); ?>" data-slide="<?php echo e($si); ?>">
                <img src="<?php echo e($slide->image_url); ?>" alt="<?php echo e($slide->title ?? 'Slide '.($si+1)); ?>" loading="lazy" decoding="async" class="w-full h-full object-cover opacity-30 object-top">
        ...
        <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-2" id="slideDots">
            <?php $__currentLoopData = $sliderItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $si => $slide): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button class="slide-dot w-2 h-2 rounded-full transition-all <?php echo e($si === 0 ? 'bg-emerald-400 w-6' : 'bg-white/30 hover:bg-white/60'); ?>" data-dot="<?php echo e($si); ?>"></button>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    <?php else: ?>
        <div class="absolute inset-0">
            <?php if($heroImage): ?>
                <img src="<?php echo e($heroImage); ?>" alt="Hero" loading="lazy" decoding="async" class="w-full h-full object-cover opacity-30 object-top">
            <?php endif; ?>
            <div class="absolute inset-0 bg-gradient-to-t from-[#022c22] via-[#022c22]/60 to-transparent"></div>
        </div>
    <?php endif; ?>
    ...
    <div class="max-w-7xl mx-auto px-6 relative z-10 flex flex-col items-center text-center">
        ...
        <h1 class="text-3xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-orange-300 to-emerald-400 leading-tight mb-8 md:mb-12 mt-10 md:mt-20 max-w-4xl tracking-tight">
            <?php echo e($siteSettings['hero_title'] ?? 'Wujudkan Dampak Nyata, Bersama Kita Bisa.'); ?>

        </h1>

        <p class="text-base md:text-lg text-emerald-50/70 mb-10 md:mb-16 max-w-2xl leading-relaxed">
            <?php echo e($siteSettings['hero_subtitle'] ?? 'Platform donasi yang transparan dan dapat dipantau. Bergabunglah bersama ribuan donatur yang sudah bergerak nyata untuk mereka yang membutuhkan.'); ?>

        </p>

        <div class="flex flex-col sm:flex-row items-center gap-4 w-full justify-center">
            <a href="<?php echo e(route('landing.programs')); ?>" class="w-full sm:w-auto px-8 py-3.5 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-extrabold rounded-2xl transition-all shadow-xl shadow-emerald-500/30 flex items-center justify-center gap-3">
            ...
            <a href="<?php echo e(route('landing.about')); ?>" class="w-full sm:w-auto px-8 py-3.5 bg-white/5 hover:bg-white/10 text-white font-bold rounded-2xl backdrop-blur-md border border-white/10 transition-all flex items-center justify-center gap-3">
                <?php if (isset($component)) { $__componentOriginal2a408760c50f134c49145997aa7ece00 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal2a408760c50f134c49145997aa7ece00 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.icons.person-circle','data' => ['class' => 'h-5 w-5']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('icons.person-circle'); ?>
...
<?php endif; ?>
<?php if (isset($__attributesOriginal2a408760c50f134c49145997aa7ece00)): ?>
<?php $attributes = $__attributesOriginal2a408760c50f134c49145997aa7ece00; ?>
<?php unset($__attributesOriginal2a408760c50f134c49145997aa7ece00); ?>
<?php endif; ?>
<?php if (isset($__componentOriginal2a408760c50f134c49145997aa7ece00)): ?>
<?php $component = $__componentOriginal2a408760c50f134c49145997aa7ece00; ?>
<?php unset($__componentOriginal2a408760c50f134c49145997aa7ece00); ?>
<?php endif; ?>
                Lihat Profil Kami
            </a>
        </div>
        ...
        <div class="mt-20 pt-8 border-t border-white/10 flex flex-wrap justify-center gap-8 md:gap-16">
            <div class="text-left">
                <p class="text-3xl md:text-4xl font-extrabold text-white"><?php echo e($heroDonorLabel); ?></p>
                <p class="text-orange-400 text-[11px] md:text-xs font-bold tracking-[0.2em] uppercase mt-1">Donatur Aktif</p>
            </div>
            <div class="text-left">
                <p class="text-3xl md:text-4xl font-extrabold text-white"><?php echo e($heroAmountLabel); ?></p>
                <p class="text-orange-400 text-[11px] md:text-xs font-bold tracking-[0.2em] uppercase mt-1">Telah Disalurkan</p>
            </div>
            <div class="text-left">
                <p class="text-3xl md:text-4xl font-extrabold text-white"><?php echo e($heroProgramLabel); ?></p>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.icons.arrow-right','data' => ['class' => 'h-5 w-5']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
            ...
            <?php else: ?>
                <?php $__currentLoopData = $homePartners; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $partner): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="group relative flex flex-col items-center justify-center grayscale hover:grayscale-0 transition-all duration-300">
                        <img src="<?php echo e($partner->logo); ?>" alt="<?php echo e($partner->name); ?>" loading="lazy" decoding="async" class="h-10 md:h-12 object-contain transition-transform duration-300 group-hover:scale-110">
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php endif; ?>
        </div>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.landing.cta','data' => ['title' => 'Jadilah Relawan Lapangan Kami','description' => 'Bantu kami di lapangan — mulai dari pengepakan logistik hingga penyaluran bantuan langsung ke tangan penerima manfaat.','buttonText' => 'Gabung Jadi Relawan','buttonUrl' => ''.e(route('landing.get-involved')).'','secondaryButtonText' => 'Tanya Jawab (FAQ)','secondaryButtonUrl' => ''.e(route('landing.faq')).'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Jadilah Relawan Lapangan Kami','description' => 'Bantu kami di lapangan — mulai dari pengepakan logistik hingga penyaluran bantuan langsung ke tangan penerima manfaat.','buttonText' => 'Gabung Jadi Relawan','buttonUrl' => ''.e(route('landing.get-involved')).'','secondaryButtonText' => 'Tanya Jawab (FAQ)','secondaryButtonUrl' => ''.e(route('landing.faq')).'']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
                ...
                <form id="homeContactForm" method="POST" action="<?php echo e(route('contact.store')); ?>" class="flex flex-col gap-6 animate-fade-in">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" value="<?php echo e(old('name')); ?>" required placeholder="Nama Anda atau Organisasi" class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="email" name="email" value="<?php echo e(old('email')); ?>" required placeholder="alamat@email.com" class="w-full pl-11 pr-4 py-3 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 mb-1.5 tracking-wide">Isi Pesan</label>
                        <textarea name="message" rows="4" required placeholder="Tuliskan tujuan / masalah yang ingin didiskusikan..." class="w-full p-4 bg-slate-50 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-emerald-500/20 outline-none text-sm transition-all resize-none"><?php echo e(old('message')); ?></textarea>
                    </div>
...
<?php echo $__env->make('layouts.landing', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\coding\fundunity\resources\views/landing/home.blade.php ENDPATH**/ ?>