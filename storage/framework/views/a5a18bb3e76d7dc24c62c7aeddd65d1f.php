    ...
    <div class="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white">
      ...
      <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
        <div class="relative w-full sm:w-auto flex-1 sm:flex-none">
          ...
          <input id="memberSearch" type="text" placeholder="Cari nama atau jabatan..." class="w-full sm:w-64 pl-10 pr-4 py-2.5 border border-admin-500 text-admin-900 rounded-xl text-sm bg-white focus:ring-4 focus:ring-admin-500/20 outline-none transition-all placeholder:text-admin-500/50 shadow-sm">
        </div>
        <button id="openMemberAdd" class="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 bg-admin-600 text-white rounded-lg text-xs font-bold hover:bg-admin-700 transition-colors shadow-sm whitespace-nowrap">
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-admin-600">
            <th class="py-4 px-6 text-left text-[11px] font-bold text-white border-b border-admin-100/50">Jabatan</th>
            <th class="py-4 px-6 text-left text-[11px] font-bold text-white border-b border-admin-100/50">Nama</th>
            <th class="py-4 px-6 text-left text-[11px] font-bold text-white border-b border-admin-100/50">Keterangan</th>
            <th class="py-4 px-6 text-center text-[11px] font-bold text-white border-b border-admin-100/50 w-24">Foto</th>
            <th class="py-4 px-6 text-center text-[11px] font-bold text-white border-b border-admin-100/50 w-24">Aksi</th>
          </tr>
        </thead>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.modal','data' => ['id' => 'memberEditModal','title' => 'Edit Anggota','maxWidth' => 'max-w-lg','headerColor' => 'bg-admin-600','closeButtonId' => 'closeMemberEdit']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['id' => 'memberEditModal','title' => 'Edit Anggota','maxWidth' => 'max-w-lg','headerColor' => 'bg-admin-600','closeButtonId' => 'closeMemberEdit']); ?>
    <form id="memberEditForm" class="flex flex-col flex-1 overflow-hidden">
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        <div>
          ...
          <input id="editJabatan" required class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Nama</label>
          <input id="editNama" required class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Bio / Keterangan</label>
          <textarea id="editDesc" rows="3" class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none"></textarea>
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Foto Profil</label>
          <input id="editImageFile" type="file" accept="image/*" class="w-full text-xs file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-admin-50 file:text-admin-700 hover:file:bg-admin-100">
        </div>
      </div>
...
<?php if (isset($component)) { $__componentOriginal883972b03e56cea0994a1aaccc5761f0 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal883972b03e56cea0994a1aaccc5761f0 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.admin.modal','data' => ['id' => 'memberAddModal','title' => 'Tambah Anggota','maxWidth' => 'max-w-lg','headerColor' => 'bg-admin-600','closeButtonId' => 'closeMemberAdd']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('admin.modal'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['id' => 'memberAddModal','title' => 'Tambah Anggota','maxWidth' => 'max-w-lg','headerColor' => 'bg-admin-600','closeButtonId' => 'closeMemberAdd']); ?>
    <form id="memberAddForm" class="flex flex-col flex-1 overflow-hidden">
      <div class="p-6 space-y-4 overflow-y-auto flex-1">
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Jabatan</label>
          <input id="addJabatan" required class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none" placeholder="Contoh: Ketua Umum">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Nama</label>
          <input id="addNama" required class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none" placeholder="Masukkan nama lengkap...">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Bio / Keterangan</label>
          <textarea id="addDesc" rows="3" class="w-full border border-slate-200 rounded-xl px-4 py-2 bg-slate-50 text-sm focus:border-admin-500 outline-none" placeholder="Tuliskan keterangan singkat..."></textarea>
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-500 mb-1">Foto Profil</label>
          <input id="addImageFile" type="file" accept="image/*" class="w-full text-xs file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-admin-50 file:text-admin-700 hover:file:bg-admin-100">
        </div>
      </div>
      <div class="p-5 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 rounded-b-3xl shrink-0">
  ...
  function renderMemberRows() {
    ...
    document.getElementById('memberRows').innerHTML = data.length ? data.map((item) => `
      <tr class="hover:bg-slate-50/50 transition-colors">
        <td class="px-6 py-5 align-top"><span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-admin-50 text-admin-700 text-xs font-bold border border-admin-100"><i class="ph ph-identification-badge"></i> ${item.jabatan}</span></td>
        ...
        <td class="p-5"><div class="flex items-center justify-center gap-2"><button class="px-3 py-1.5 bg-admin-600 text-white font-bold rounded-lg text-[11px] hover:bg-admin-700 transition-colors" onclick="openEditMember(${item.id})">Edit</button><button class="px-3 py-1.5 bg-rose-600 text-white font-bold rounded-lg text-[11px] hover:bg-rose-700 transition-colors" onclick="openDeleteMember(${item.id})">Hapus</button></div></td>
      </tr>
