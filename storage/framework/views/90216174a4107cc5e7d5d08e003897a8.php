...
<div id="incomeModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in">
...
<script>
  ...
  document.getElementById('openIncomeModal').addEventListener('click', () => document.getElementById('incomeModal').classList.remove('hidden'));
  document.getElementById('closeIncomeModal').addEventListener('click', () => document.getElementById('incomeModal').classList.add('hidden'));
  document.getElementById('cancelIncomeModal').addEventListener('click', () => document.getElementById('incomeModal').classList.add('hidden'));
  document.getElementById('saveIncomeModal').addEventListener('click', () => { document.getElementById('incomeModal').classList.add('hidden'); alert('Transaksi manual tersimpan.'); });
...
<?php echo $__env->make('layouts.admin.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH F:\Magang\PT. YMP\fundunity\resources\views/admin/keuangantransparansi.blade.php ENDPATH**/ ?>